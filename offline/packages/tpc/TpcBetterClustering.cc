...
TpcBetterClustering::TpcBetterClustering(const string &name)
  : TpcClustering(name)
...
//===================
bool TpcBetterClustering::is_local_maximum(int phibin, int zbin, std::vector<std::vector<double>> &adcval)
{
  bool retval = true;
...
bool TpcBetterClustering::is_in_sector_boundary(int phibin, int sector, PHG4CylinderCellGeom *layergeom)
{
...
void TpcBetterClustering::remove_hit(double adc, int phibin, int zbin, std::multimap<double, ihit> &all_hit_map, std::vector<std::vector<double>> &adcval)
{
...
void TpcBetterClustering::remove_hits(std::vector<ihit> &ihit_list, std::multimap<double, ihit> &all_hit_map,std::vector<std::vector<double>> &adcval )
{
...
void TpcBetterClustering::calc_cluster_parameter(std::vector<ihit> &ihit_list,int iclus, PHG4CylinderCellGeom *layergeom, TrkrHitSet *hitset)
{

  ///HERE TODO
  //cout << "TpcBetterClustering: process cluster iclus = " << iclus <<  " in layer " << layer << endl;
...
void TpcBetterClustering::print_cluster(std::vector<ihit> &ihit_list)
{
  for(auto iter = ihit_list.begin(); iter != ihit_list.end();++iter){
    double adc = iter->first; 
    int phibin = iter->second.first;
...
int TpcBetterClustering::Process(PHCompositeNode *topNode)
{
  int print_layer = 18;

  if (Verbosity() > 1000)
    std::cout << "TpcBetterClustering::Process_Event" << std::endl;
