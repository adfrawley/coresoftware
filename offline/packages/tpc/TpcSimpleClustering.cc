...
TpcSimpleClustering::TpcSimpleClustering(const string &name)
  : TpcClustering(name)
...
//===================
bool TpcSimpleClustering::is_local_maximum(int phibin, int zbin, std::vector<std::vector<double>> &adcval)
{
  bool retval = true;
...
bool TpcSimpleClustering::is_in_sector_boundary(int phibin, int sector, PHG4CylinderCellGeom *layergeom)
{
...
void TpcSimpleClustering::remove_hit(double adc, int phibin, int zbin, std::multimap<double, ihit> &all_hit_map, std::vector<std::vector<double>> &adcval)
{
...
void TpcSimpleClustering::remove_hits(std::vector<ihit> &ihit_list, std::multimap<double, ihit> &all_hit_map,std::vector<std::vector<double>> &adcval )
{
...
void TpcSimpleClustering::calc_cluster_parameter(std::vector<ihit> &ihit_list,int iclus, PHG4CylinderCellGeom *layergeom, TrkrHitSet *hitset)
{

  ///HERE TODO
  //cout << "TpcSimpleClustering: process cluster iclus = " << iclus <<  " in layer " << layer << endl;
...
void TpcSimpleClustering::print_cluster(std::vector<ihit> &ihit_list)
{
  for(auto iter = ihit_list.begin(); iter != ihit_list.end();++iter){
    double adc = iter->first; 
    int phibin = iter->second.first;
...
void TpcSimpleClustering::find_z_range(int phibin, int zbin, std::vector<std::vector<double>> &adcval, int& zdown, int& zup){
...
void TpcSimpleClustering::find_phi_range(int phibin, int zbin, std::vector<std::vector<double>> &adcval, int& phidown, int& phiup){
...
void TpcSimpleClustering::get_cluster(int phibin, int zbin, std::vector<std::vector<double>> &adcval, std::vector<ihit> &ihit_list)
{
...
int TpcSimpleClustering::Process(PHCompositeNode *topNode)
{
  int print_layer = 18;
       ...
       ++hitsetitr)
  {
    ...
    if (Verbosity() > 2)
      ...
      {
	cout << endl << "TpcSimpleClustering process hitsetkey " << hitsetitr->first
