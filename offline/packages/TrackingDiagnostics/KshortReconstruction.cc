...
int KshortReconstruction::process_event(PHCompositeNode* topNode)
{
  ...
  for (auto tr1_it = m_svtxTrackMap->begin(); tr1_it != m_svtxTrackMap->end(); ++tr1_it)
  {
    ...
    // get the dedx
    auto *tpcseed1 = tr1->get_tpc_seed();
    float dedx1 = 0;
    if (tpcseed1)
      {
	dedx1 = calc_dedx(tpcseed1, m_clustermap, m_tpcGeom);
      }
    ...
    for (auto tr2_it = std::next(tr1_it); tr2_it != m_svtxTrackMap->end(); ++tr2_it)
    {
      ...
      // get the dedx
      auto *tpcseed2 = tr2->get_tpc_seed();
      float dedx2 = 0;
      if (tpcseed2)
	{
	  dedx2 = calc_dedx(tpcseed2, m_clustermap, m_tpcGeom);
	}
...
void KshortReconstruction::fillNtp(SvtxTrack* track1, SvtxTrack* track2, float mass1, float mass2, float dedx1, float dedx2, Acts::Vector3 dcavals1, Acts::Vector3 dcavals2, Acts::Vector3 pca_rel1, Acts::Vector3 pca_rel2, double pair_dca, double invariantMass, double invariantPt, float invariantPhi, float rapidity, float pseudorapidity, Eigen::Vector3d projected_pos1, Eigen::Vector3d projected_pos2, Eigen::Vector3d projected_mom1, Eigen::Vector3d projected_mom2, Acts::Vector3 pca_rel1_proj, Acts::Vector3 pca_rel2_proj, double pair_dca_proj, unsigned int track1_silicon_cluster_size, unsigned int track2_silicon_cluster_size, unsigned int track1_mvtx_cluster_size,  unsigned int track1_mvtx_state_size, unsigned int track1_intt_cluster_size,  unsigned int track1_intt_state_size, unsigned int track2_mvtx_cluster_size,  unsigned int track2_mvtx_state_size, unsigned int track2_intt_cluster_size,  unsigned int track2_intt_state_size, int runNumber, int eventNumber)
{
  ...
  if (svtxVertex)
  {
    ...
    ntracks_vertex = svtxVertex->size_tracks();
  }
...
void KshortReconstruction::fillHistogram(Eigen::Vector3d mom1, Eigen::Vector3d mom2, TH1D* massreco, double& invariantMass, double& invariantPt, float& invariantPhi, float& rapidity, float& pseudorapidity, float &decaymassa, float &decaymassb)
{
  ...
  if (Verbosity() > 1)
    {
      std::cout << "px1: " << mom1(0) << " py1: " << mom1(1) << " pz1: " << mom1(2) << " mass " << decaymassa << " E1: " << E1 << std::endl;
      std::cout << "px2: " << mom2(0) << " py2: " << mom2(1) << " pz2: " << mom2(2) << "  mass2 " << decaymassb << " E2: " << E2 << std::endl;
...
float KshortReconstruction::calc_dedx(TrackSeed* tpcseed, TrkrClusterContainer* clustermap, PHG4TpcCylinderGeomContainer* tpcGeom)
{
